...
def objective_function(x, y, z):
    # Calculate the first term of the Ackley function
    term1 = -20 * np.exp(-0.2 * np.sqrt((x**2 + y**2 + z**2) / 3))
    # Calculate the second term of the Ackley function
    term2 = -np.exp((np.cos(2 * np.pi * x) + np.cos(2 * np.pi * y) + np.cos(2 * np.pi * z)) / 3)
    # The Ackley function with added constants
    return term1 + term2 + 20 + np.exp(1)

# Calculate fitness as the inverse of the objective function, shifted by +1
def fitness(x, y, z):
    return 1 / (objective_function(x, y, z) + 1)
...
# Select parents using tournament selection method
...
def evolutionary_algorithm(pop_size=50, num_generations=100):#Population size and number of generations are default 50 and 100
    ...
    for generation in range(num_generations):
        ...
        best_fitness = np.max([fitness(ind[0], ind[1], ind[2]) for ind in population])
